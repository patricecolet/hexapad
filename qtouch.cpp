...
void NoteQtouch::update(PadSettings pad) {
  ...
  if((qt_measure > qt_floor + advancedSettings.qtouchRoundOff) && qt_memory == 0) { // Si la valeur est suppérieur au seuil
...
//  if((qt_measure < (qt_floor + advancedSettings.roundOff)) && qt_memory != 0) qt_memory = 0;
  if((qt_measure < (qt_floor + advancedSettings.qtouchRoundOff)) && qt_memory != 0) {  // Si la valeur est inférieur au seuil
...
int NoteQtouch::getTouch() {
  int qt_measure = qt.measure();
  int range = 1014 - qt_floor + advancedSettings.qtouchRoundOff; // Calcul seuil
  velocity = 127 * (qt_measure - qt_floor + advancedSettings.qtouchRoundOff) / range;
